...
energyact_fin <- energyact_fin %>%
  mutate(category = case_when(
    max_similarity_econ >= 0.7 & max_similarity_econ > max_similarity_env ~ "econ",
    max_similarity_env >= 0.7 & max_similarity_env > max_similarity_econ ~ "env",
    TRUE ~ "rest"
  ))
```
...
ggsave(here::here("plot","H1_0.7","econ_subset_voters_barplot.png"), plot = econ_subset_voters_barplot, width = 10, height = 8, dpi = 300, bg = "transparent")
```
...
ggsave(here::here("plot","H1_0.7","env_subset_voters_barplot.png"), plot = env_subset_voters_barplot, width = 10, height = 8, dpi = 300, bg = "transparent")
```
...
#env 0 significant
# Calculate median values for 'env' at t1 and t2 within data_vote_0
median_env_t1 <- median(data_vote_0$env[data_vote_0$wave == "t1"], na.rm = TRUE)
median_env_t2 <- median(data_vote_0$env[data_vote_0$wave == "t2"], na.rm = TRUE)

# Print median values
print(paste("Median env at t1:", median_env_t1))
print(paste("Median env at t2:", median_env_t2))

# Determine direction of effect
if (median_env_t1 < median_env_t2) {
  print("The environment variable tends to increase from t1 to t2.")
} else if (median_env_t1 > median_env_t2) {
  print("The environment variable tends to decrease from t1 to t2.")
} else {
  print("There is no change in the environment variable from t1 to t2.")
}

wilcox_effsize(data_vote_0, env~wave,paired=T)
wilcox_effsize(data_vote_0, econ~wave,paired=T)
wilcox_effsize(data_vote_1, env~wave,paired=T)
wilcox_effsize(data_vote_1, econ~wave,paired=T)
```
...
# Plot the ROC curve
auc_plot_0.7 <- plot(roc_curve, main="ROC Curve")
