...
```
```{r}
# Subset for 'for' and 'against' voters, excluding H1 == 3 and H1 == 4
for_voters <- subset(energyact_fin_updated, intendedVote == 1 & !(H1 %in% c(3, 4)))
against_voters <- subset(energyact_fin_updated, intendedVote == 0 & !(H1 %in% c(3, 4)))

# Calculate percentages for economy words among 'for' and 'against' voters
economy_words_for <- sum(for_voters$H1 == 1)
