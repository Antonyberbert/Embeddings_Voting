...
embeddings_new <- read.csv(here("data","features_3.csv"))
...
climateact_new <- cbind(structure, select(embeddings_new,-1))
...
climateact_word <- left_join(climateact_word, 
                        select(additional_data, participantID, intendedVote, ratingLaw, mean_valence_macro), 
                        by = c("personID" = "participantID"))

climateact_new <- left_join(climateact_new, 
                        select(additional_data, participantID, intendedVote, ratingLaw, mean_valence_macro), 
                        by = c("personID" = "participantID"))
...
cols_word <- c(1:10, (ncol(climateact_word)-2):ncol(climateact_word), 11:(ncol(climateact_word)-3))
cols_new <- c(1:10, (ncol(climateact_new)-2):ncol(climateact_new), 11:(ncol(climateact_new)-3))


climateact <- climateact[, cols]

climateact_word <- climateact_word[, cols_word]

climateact_new <- climateact_new[, cols_new]
...
embedding_cols_new <- paste("X", 0:1023, sep = "")
...
#4 Visualize new embeddings
embeddings_climateact_new <- as.matrix(climateact_new[, embedding_cols_new])
# Perform t-SNE
set.seed(123) # for reproducibility
tsne_climateact_new <- Rtsne(embeddings_climateact_new, is_distance = FALSE, perplexity = 30, check_duplicates = FALSE)

# Plot t-SNE results
plot4 <- ggplot(NULL, aes(x = tsne_climateact_new$Y[, 1], y = tsne_climateact_new$Y[, 2])) +
  geom_point(alpha = 0.6) +
  labs(title = "t-SNE of new embeddings")
...
#UMAP new embeddings
scaled_embeddings_new <- scale(embeddings_climateact_new)
#Change Arguments of umap
custom.settings <- umap.defaults
custom.settings$random_state = 26 #set seed for reproducibility
custom.settings$transform_state = 26 #set seed for reproducibility
...
#Perform dimensionality reduction on embeddings with umap
umap_result_new <- umap::umap(scaled_embeddings_new,custom.settings)

#Visualize the results
plot(umap_result_new$layout[,1],umap_result_new$layout[,2], main = "UMAP Reduction new", xlab="UMAP 1", ylab = "UMAP 2")
```
...
#--------------------------------------------------------------------------------
#Cluster umap new embeddings
db_result_umap_new <- dbscan::dbscan(umap_result_new$layout, eps = 12, minPts = 5)

# Count of data points in each cluster
cluster_counts_umap_new <- table(db_result_umap_new$cluster)

# Display the clustering results
print(cluster_counts_umap_new)

# Number of clusters (excluding noise)
num_clusters_umap_new <- length(unique(db_result_umap_new$cluster)) - 1  # Subtracting 1 to exclude noise
cat("Number of clusters:", num_clusters_umap_new, "\n")

# Number of noise points
num_noise_umap_new <- sum(db_result_umap_new$cluster == 0)
cat("Number of noise points:", num_noise_umap_new, "\n")
```
...
# Applying PCA
scaled_embeddings_new <- scale(embeddings_climateact_new)
pca_result_new <- prcomp(scaled_embeddings_new, center = TRUE, scale. = TRUE)

# Plotting the variance explained by each principal component
explained_variance_ratio_new <- pca_result_new$sdev^2 / sum(pca_result_new$sdev^2)
cum_explained_variance_new <- cumsum(explained_variance_ratio_new)
plot(explained_variance_ratio_new, type = "b", main="Explained Variance by Principal Components",
     xlab = "Principal Component", ylab = "Explained Variance Ratio")

plot(cum_explained_variance_new, type = "b", main="Cumulative Explained Variance",
     xlab = "Number of Principal Components", ylab = "Cumulative Explained Variance Ratio",
     ylim=c(0.8,1), yaxp=c(0.8,1,4))  # Here, it's divided into 0.8, 0.85, 0.9, 0.95, 1
...
pca_transformed_data_new <- pca_result_new$x[, 1:300]
```

```{r}
k <- 300
knn_dist <- dbscan::kNNdist(pca_transformed_data_new, k = k)
...
```{r,eval=F, echo=F}
# Apply DBSCAN on PCA reduced set
db_result_new <- dbscan::dbscan(pca_transformed_data_new, eps = 30, minPts = 10)

# Count of data points in each cluster
cluster_counts_new <- table(db_result_new$cluster)

# Display the clustering results
print(cluster_counts_new)

# Number of clusters (excluding noise)
num_clusters_new <- length(unique(db_result_new$cluster)) - 1  # Subtracting 1 to exclude noise
cat("Number of clusters:", num_clusters_new, "\n")

# Number of noise points
num_noise_new <- sum(db_result_new$cluster == 0)
cat("Number of noise points:", num_noise_new, "\n")
```
...
# Apply DBSCAN on PCA reduced set
set.seed(123)
db_result_new2 <- dbscan::dbscan(pca_transformed_data_new, eps = 15, minPts = 5)

# Count of data points in each cluster
cluster_counts_new2 <- table(db_result_new2$cluster)

# Display the clustering results
print(cluster_counts_new2)

# Number of clusters (excluding noise)
num_clusters_new2 <- length(unique(db_result_new2$cluster)) - 1  # Subtracting 1 to exclude noise
cat("Number of clusters:", num_clusters_new2, "\n")

# Number of noise points
num_noise_new2 <- sum(db_result_new2$cluster == 0)
cat("Number of noise points:", num_noise_new2, "\n")
```

```{r, eval=F, echo=F}
#Trying out different parameters
# Apply DBSCAN on PCA reduced set
db_result_new3 <- dbscan::dbscan(pca_transformed_data_new, eps = 23, minPts = 5)

# Count of data points in each cluster
cluster_counts_new3 <- table(db_result_new3$cluster)

# Display the clustering results
print(cluster_counts_new3)

# Number of clusters (excluding noise)
num_clusters_new3 <- length(unique(db_result_new3$cluster)) - 1  # Subtracting 1 to exclude noise
cat("Number of clusters:", num_clusters_new3, "\n")

# Number of noise points
num_noise_new3 <- sum(db_result_new3$cluster == 0)
cat("Number of noise points:", num_noise_new3, "\n")
```

```{r, eval=F, echo=F}
#Trying out different parameters
# Apply DBSCAN on PCA reduced set
db_result_new4 <- dbscan::dbscan(pca_transformed_data_new, eps = 25, minPts = 3)

# Count of data points in each cluster
cluster_counts_new4 <- table(db_result_new4$cluster)

# Display the clustering results
print(cluster_counts_new4)

# Number of clusters (excluding noise)
num_clusters_new4 <- length(unique(db_result_new4$cluster)) - 1  # Subtracting 1 to exclude noise
cat("Number of clusters:", num_clusters_new4, "\n")

# Number of noise points
num_noise_new4 <- sum(db_result_new4$cluster == 0)
cat("Number of noise points:", num_noise_new4, "\n")
```
...
#Visualize new embeddings umap
# Create a dataframe for plotting
plot_df_umap_new <- data.frame(umap_result_new$layout)
colnames(plot_df_umap_new) <- c("UMAP1", "UMAP2")
plot_df_umap_new$cluster <- as.factor(db_result_umap_new$cluster)

# Plot the UMAP results colored by cluster assignment
ggplot(plot_df_umap_new, aes(x = UMAP1, y = UMAP2, color = cluster)) +
  geom_point(alpha = 0.6)+
  labs(title = "UMAP visualization with DBSCAN clustering", color = "Cluster") +
  theme_minimal()
```
...
```{r,eval=F,echo=F}
#Plot PCA new embeddings
# Create a dataframe for plotting
plot_data_new <- data.frame(pca_transformed_data_new, Cluster = as.factor(db_result_new$cluster))

# Filter out the noise cluster
plot_data_filtered_new <- plot_data_new %>% filter(Cluster != 0)

# Add cluster assignment to the dataframe
plot_data_filtered_new$Cluster <- db_result_new$cluster[db_result_new$cluster != 0]

# Visualize the clusters using ggplot2 with jitter
ggplot(plot_data_filtered_new, aes(x = PC1, y = PC2, color = as.factor(Cluster))) +  
  geom_point(size = 2, alpha = 0.5) + 
  labs(title = "PCA visualization with DBSCAN clustering (excluding noise)",
       x = "PC1",
       y = "PC2",
...
#Climateact_word_selected$new_umap_cluster <- db_result_umap_new$cluster
#Climateact_word_selected$new_pca_cluster <- db_result_new$cluster
climateact_word_selected$new2_pca_cluster <- db_result_new2$cluster
#Climateact_word_selected$new3_pca_cluster <- db_result_new3$cluster


# Rename the resulting dataframe
climateact_word_clustered <- climateact_word_selected
write_xlsx(climateact_word_clustered, "Climateact_word_clustered.xlsx")

View(climateact_word_clustered)
```

```{r}
#Creating new df with the 0 clusters to cluster them again separately.
climateact_rest <- cbind(climateact_word[,2:12],climateact_word_selected$new2_pca_cluster,embeddings_climateact_new)
climateact_rest <- climateact_rest%>%
  filter(climateact_word_selected$new2_pca_cluster==0)
embeddings_rest <- as.matrix(climateact_rest[, embedding_cols_new])

# Perform t-SNE
set.seed(123) # for reproducibility
tsne_climateact_rest <- Rtsne(embeddings_rest, is_distance = FALSE, perplexity = 30, check_duplicates = FALSE)

# Plot t-SNE results
plot_rest <- ggplot(NULL, aes(x = tsne_climateact_rest$Y[, 1], y = tsne_climateact_rest$Y[, 2])) +
  geom_point(alpha = 0.6) +
...
# Applying PCA
scaled_embeddings_rest <- scale(embeddings_rest)
pca_result_rest <- prcomp(scaled_embeddings_rest, center = TRUE, scale. = TRUE)
...
```{r}
# Project the data onto the first 300 principal components
pca_transformed_data_rest <- pca_result_rest$x[, 1:300]
```

```{r}
# Apply DBSCAN on PCA reduced set
db_result_rest <- dbscan::dbscan(pca_transformed_data_rest, eps = 20, minPts = 3)
...
# Extract columns 2:12 from the climateact_word dataset
climateact_word_rest <- climateact_rest[, 2:12]
...
#Climateact_word_selected$new_umap_cluster <- db_result_umap_new$cluster
#Climateact_word_selected$new_pca_cluster <- db_result_new$cluster
climateact_word_rest$rest_cluster <- db_result_rest$cluster
#Climateact_word_selected$new3_pca_cluster <- db_result_new3$cluster


#Write Xl file
write_xlsx(climateact_word_rest, "Climateact_word_rest.xlsx")
```
